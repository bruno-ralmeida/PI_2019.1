	...
	<div id="main" class="container-fluid" style="padding-top: 2%">
		...
		<div class="row-center">
			...
			<address>
				...
				<p><strong>Mátricula: </strong>${professor.matricula }</p>
				<p><strong>E-Mail: </strong>${professor.email }</p>
				<c:if test="${professor.administrador == 1 }">
					<p><strong>Administrador: </strong>Sim</p>
				</c:if>
				<c:if test="${professor.administrador == 0 }">
					<p><strong>Administrador: </strong>Não</p>
				</c:if>
				</address>
			</div>
		</div>
		...
		<div id="actions" class="row">
			...
			<a href="professor.jsp" class="btn btn-default" style="width: 25%;">Voltar</a>
				<button type="button" class="btn  btn-xs"
										data-toggle="modal" data-target="#modalU-${professor.id}" style="width: 25%; background-color: #00458a; color: white;">Editar</button>
									<button type="button" class="btn btn-danger btn-xs"
										data-toggle="modal" data-target="#modal-${professor.id}" style="width: 25%;  ">Excluir</button>
		...
		<div class="modal fade" id="modalU-${professor.id}"
			...
			<div class="modal-dialog modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">Deseja realmente alterar as informações do professor(a)
						${professor.nome}?</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary"
							data-dismiss="modal">Cancelar</button>
						<a class="btn btn-xs"
							href="ManterProfessorController.do?acao=Editar&id=${professor.id }" style="width: 25%; background-color: #00458a; color: white;">Confirmar</a>
					</div>
				</div>
			</div>
		</div>
